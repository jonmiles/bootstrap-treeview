...
;(function ($, window, document, undefined) {
	...
	Tree.prototype._subscribeEvents = function () {
		...
		if (typeof (this._options.onNodeIntermediate) === 'function') {
			this.$element.on('nodeIntermediate', this._options.onNodeIntermediate);
		}
	...
	*/
	Tree.prototype._setInitialStates = function (node, level) {
		...
		$.each(node.nodes, function checkStates(index, node) {
			...
			if (!node.hasOwnProperty('selectable')) {
				node.selectable = _this._options.enableSelectable;
			}
	...
	Tree.prototype._clickHandler = function (event) {
		...
		if ((classList.indexOf('expand-icon') !== -1)) {
			...
			if (this._options.enableSelectable && node.selectable) {
		...
		if (state) {
			...
			if (node.$el) {
				...
				node.$el.children('span.check-icon')
					...
					.removeClass(this._options.intermediateIcon)
			...
			if (node.$el) {
				...
				node.$el.children('span.check-icon')
					.removeClass(this._options.uncheckedIcon)
					.removeClass(this._options.checkedIcon)
					.addClass(this._options.intermediateIcon);
			}
	...
	*/
	Tree.prototype.getChecked = function (parentNodeNotAllowed) {
		return this._findNodes('true', 'g', 'state.checked', parentNodeNotAllowed);
	};
	...
	*/
	Tree.prototype.getUnchecked = function (parentNodeNotAllowed) {
		return this._findNodes('false', 'g', 'state.checked', parentNodeNotAllowed);
	};
	...
	*/
	Tree.prototype.checkAll = function (options) {
		var identifiers = this._findNodes('false|null', 'g', 'state.checked');
		this._forEachIdentifier(identifiers, options, $.proxy(function (node, options) {
	...
	*/
	Tree.prototype.uncheckAll = function (options) {
		var identifiers = this._findNodes('true|null', 'g', 'state.checked');
		this._forEachIdentifier(identifiers, options, $.proxy(function (node, options) {
			this._setChecked(node, false, options);
		}, this));
	};
	...
	*/
	Tree.prototype.intermediateNode = function (identifiers, options) {
		this._forEachIdentifier(identifiers, options, $.proxy(function (node, options) {
			this._setChecked(node, null, options);
		}, this));
	};
	...
	*/
	Tree.prototype._findNodes = function (pattern, modifier, attribute, parentNodeNotAllowed) {
